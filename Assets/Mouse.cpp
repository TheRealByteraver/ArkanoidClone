...
bool MouseClient::IsInArea( Rect area ) const
{
    return ((server.x >= area.x1) &&
            (server.x <= area.x2) &&
            (server.y >= area.y1) &&
            (server.y <= area.y2));
}

bool MouseClient::IsInUpperHalf( Rect area ) const
{
    return ( server.y < area.y1 + (area.y2 - area.y1) / 2 );
}
